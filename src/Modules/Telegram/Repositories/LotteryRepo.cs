...
public class LotteryRepo(MainContext context) : GenericRepository<Lottery>(context), ILotteryRepo
{
    ...
    public async Task<LotterySummaryDto?> GetSummary(string userId)
    {
        return await _store.Select(x => new LotterySummaryDto
        {
            ...
            WinnerCount = _store.Where(xx => xx.LotteryDate != null && xx.IsWinner && x.Confirmed && xx.ChannelMessageId == null).GroupBy(xx => xx.UserId).Count(),
    ...
    {
        return await _store.Include(x => x.User).Where(x => x.IsWinner && x.LotteryDate != null && x.SendToMainChannelDate == null && x.SendToArchiveChannelDate == null && x.ChannelMessageId == null && x.ArchiveMessageId == null).OrderBy(x => x.Id).ToListAsync(ct);
    }
    ...
    public async Task<LotteryChannelMessagesDto> GetChannelMessages(CancellationToken ct = default)
    {
        return new LotteryChannelMessagesDto
        {
            ...
            WinnerCount = await _store.Where(xx => xx.LotteryDate != null && xx.IsWinner && xx.Confirmed && xx.ChannelMessageId == null).GroupBy(xx => xx.UserId).CountAsync(ct)
        };
    }
